...
This is probably the most important command you will learn today. 
all about the program [here ](https://kb.iu.edu/d/acuy)
https://kb.iu.edu/d/acuy

#### Creating and naming an screen session
```
screen -S sessionID
```
...
You can give any nameto a session (the sessionID would be any name you give e.g. qualitycheck) that is meaningful for identifying the screen session. So, if you open several, you can navigate to the desired one because they can be easily identified. 
...
```
This is a very easy system that will log #ALL# from begining to end. It will create a logfile "output.0" int he folder where you first launched the screen session. **Only one big caveat. It you open another screen in the same folder it woill overwrite your file**
